...
var BubbleTree = function(config, onHover, onUnHover) {
	...
	// overwrite formatNumber function
	if (me.config.formatNumber) {
		BubbleTree.Utils.formatNumber = me.config.formatNumber;
	}
	...
	me.traverse = function(node, index) {
		...
		if (node.children.length == 0 && node.color) {
	...
	me.changeView = function(token) {
		...
		if (node !== null) {
			...
			for (i in me.displayObjects)
				me.displayObjects[i].hideFlag = true;


			if (node == root || ( node.parent == root && node.children.length == 0 ) ) {
				...
				if (node.children.length == 0) { // ..because if it has no children..
				...
				if (me.currentCenter && me.currentCenter == node.left)
					rightTurn = true;
				else if (me.currentCenter && me.currentCenter == node.right)
					leftTurn = true;
				...
				var lAmount = (node.left)?node.left.amount:0;
				var rAmount = (node.right)?node.right.amount:0;

				rad2 = 0 - Math.max(
					//hw *0.8 - tgtScale * (a2rad(node.parent.amount)+a2rad(node.amount)), // maximum visible part
					hw * 0.8 - tgtScale
						* (a2rad(node.parent.amount)
							+ a2rad(
								Math.max(
									node.amount * 1.15
									+ node.maxChildAmount*1.15,
									lAmount * 0.85 ,
									rAmount * 0.85
								)
							)
				...
				if (node.left && node.right) {
					var maxSiblSize = tgtScale * a2rad(Math.max(lAmount, rAmount));
				}
				...
				//TODO PALA, aca pasa algo raro REVISAR
	...
	*/
	...
	me.navigateTo = function(node, fromUrlChange) {
		...
		if (fromUrlChange)
			...
			me.history.load(me.getUrlForNode(node));
...
BubbleTree.Bubbles.Plain = function(node, bubblechart, origin, radius, angle, color) {
	...
	me.addOverlay = function() {
		...
		$(me.overlay.node).hover(me.onhover.bind(me));
		$(me.overlay.node).unhover(me.onunhover.bind(me));
