presuApp.factory('dataHelperService', function($http, $rootScope) {
  ...
  var defaultConceptos = [
    ...
    { name: 'sancionado', field: 'Sancionado', title: 'Sancionado 2015', isBase: false, clazz: 'progress-bar-danger',
      description: 'El total de crédito sancionado para todo el año.' },
  ...
  *
  ...
  dataHelperService.loadData = function(data, datacsv, dimensiones, conceptos, concepto, mes) {
    ...
    // si no especifican el mes, asigno el último mes que tengo en el dataset
    // uso reduceRight para calcular max(parseInt(row.Mes));
    if (!mes) {
      mes = dataparsed.reduceRight(function(prev, curr) {
        return Math.max(prev, parseInt(curr.Mes));
      }, 0);
    }
    ...
    data.forEach(function(key, row) {
      ...
      if (row.children) {
        ...
        if (simplify && current.children.length === 1) {
          ...
          // si el hijo unico es nodo final (no tiene mas hijo)
          // traslado el value al padre y elimino el hijo
          if (!onlychild.children) {
            current.value = onlychild.value;
